											...
											<tr class="line-<?php echo "$productId";?>">
												...
												<td id="title-<?php echo "$productId"; ?>"   class="tTitle" > 
													...
													<form method = "post" action = "products.php" enctype = "multipart/form-data">
														<input type = "file" name="product" style = "width: 280px;margin-top: 15px;" class="button" required><br>
														<input type = "hidden" name = "prodId"  value = "<?=$productId?>" >
														<input type="submit" value="Загрузить фото" class="button" style="margin-top: 15px;">
													</form>
<?php			
													if($_FILES && $_FILES['product'] && $_FILES['product']['tmp_name']){
														$path = "../img/subcat";
														$name = $_POST['prodId'].'.jpg';
															move_uploaded_file($_FILES['product']['tmp_name'], "$path/$name") or die(mysql_error());
...
<?php mysqli_free_result($resultCat);?>